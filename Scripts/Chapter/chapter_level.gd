...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		if key_event.pressed and not key_event.echo and key_event.keycode == TILE_DEBUG_TOGGLE_KEY:
...
func _build_parallax_background() -> void:
	...
	var texture_settings := [
		{"path": PARALLAX_TEXTURE_PATHS[0], "motion": Vector2(0.08, 0.0), "y": 610.0, "modulate": Color(0.28, 0.34, 0.42, 0.82)},
		{"path": PARALLAX_TEXTURE_PATHS[1], "motion": Vector2(0.14, 0.0), "y": 580.0, "modulate": Color(0.4, 0.5, 0.58, 0.82)},
		{"path": PARALLAX_TEXTURE_PATHS[2], "motion": Vector2(0.22, 0.0), "y": 560.0, "modulate": Color(0.6, 0.72, 0.8, 0.78)},
		{"path": PARALLAX_TEXTURE_PATHS[3], "motion": Vector2(0.32, 0.0), "y": 520.0, "modulate": Color(0.74, 0.86, 0.92, 0.74)}
	]
	...
	for texture_setting_variant: Variant in texture_settings:
		...
		layer.motion_mirroring = Vector2(float(texture.get_width()) * 0.9, 0.0)
		...
		for index: int in range(2):
			...
			sprite.position = Vector2(float(index) * float(texture.get_width()) * 0.9 - 80.0, float(texture_setting["y"]))
			sprite.scale = Vector2(0.9, 0.9)
			sprite.modulate = texture_setting["modulate"] as Color
			layer.add_child(sprite)

	_build_backdrop_shapes()


func _build_backdrop_shapes() -> void:
	var fill := Polygon2D.new()
	fill.color = Color(0.08, 0.11, 0.16, 1.0)
	fill.polygon = PackedVector2Array([
		Vector2(-420.0, -220.0),
		Vector2(level_size_pixels.x + 420.0, -220.0),
		Vector2(level_size_pixels.x + 420.0, level_size_pixels.y + 220.0),
		Vector2(-420.0, level_size_pixels.y + 220.0)
	])
	backdrop_root.add_child(fill)

	for index: int in range(20):
		var stalactite := Polygon2D.new()
		var start_x: float = lerpf(-120.0, level_size_pixels.x + 120.0, float(index) / 19.0)
		var width: float = rng.randf_range(60.0, 130.0)
		var height: float = rng.randf_range(90.0, 210.0)
		stalactite.color = Color(0.12, 0.16, 0.22, 0.84)
		stalactite.polygon = PackedVector2Array([
			Vector2(start_x, -40.0),
			Vector2(start_x + width, -40.0),
			Vector2(start_x + width * 0.46, height)
		])
		backdrop_root.add_child(stalactite)

	for index: int in range(16):
		var stalagmite := Polygon2D.new()
		var start_x: float = lerpf(-60.0, level_size_pixels.x + 60.0, float(index) / 15.0)
		var width: float = rng.randf_range(70.0, 150.0)
		var height: float = rng.randf_range(70.0, 180.0)
		stalagmite.color = Color(0.16, 0.2, 0.27, 0.7)
		stalagmite.polygon = PackedVector2Array([
			Vector2(start_x, level_size_pixels.y + 30.0),
			Vector2(start_x + width, level_size_pixels.y + 30.0),
			Vector2(start_x + width * 0.52, level_size_pixels.y - height)
		])
...
func _draw_debug_overlay() -> void:
	...
	if path_points.size() >= 2:
		...
		path_line.width = 5.0
		path_line.default_color = Color(0.86, 1.0, 0.42, 0.78)
		for point_variant: Variant in path_points:
			var point: Vector2i = point_variant as Vector2i
			path_line.add_point(_grid_to_world(point) + Vector2(16.0, 0.0))
		overlay_root.add_child(path_line)
...
func _spawn_hazard(hazard: Dictionary) -> void:
	...
	for offset_index: int in range(count):
		...
		spike.global_position = _grid_to_world(Vector2i(base_x + offset_index, base_y))
...
func _spawn_story_trigger(trigger_data: Dictionary) -> void:
	...
	area.body_entered.connect(_on_story_trigger_entered.bind(trigger_id, message, toast_type, banner))
...
func _spawn_exit_gate() -> void:
	...
	exit_gate.global_position = _grid_to_world(active_level.get("exit", Vector2i(96, 24)) as Vector2i) + Vector2(18.0, 22.0)
...
func _on_story_trigger_entered(body: Node2D, trigger_id: String, message: String, toast_type: String, banner: String) -> void:
...
func _player_spawn_clearance() -> float:
	...
	if rect_shape == null:
		return 14.0
	var scale_y: float = maxf(absf(player.scale.x), absf(player.scale.y))
	return maxf(rect_shape.size.x, rect_shape.size.y) * scale_y * 0.5 + 2.0
