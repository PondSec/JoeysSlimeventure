...
func _run() -> void:
	...
	for level_index: int in range(total_levels):
		...
		var title: String = str(progress.get_active_level_data().get("title", ""))
		...
		print(
			"VALIDATE level=%d title=%s terrain=%d decor=%d enemies=%d gates=%d invalid_tiles=%d start_y=%.1f end_y=%.1f floor=%s layer=%d status=%s"
			% [level_index + 1, title, terrain_count, decor_count, enemy_count, gate_count, invalid_tile_count, start_y, end_y, str(touched_floor), collision_layer, status]
		)
