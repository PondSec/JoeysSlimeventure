...
func _jump_towards_target() -> void:
	...
	squash_tween.tween_property(sprite, "scale", Vector2(0.2, 0.12), 0.08).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	squash_tween.tween_property(sprite, "scale", Vector2(0.16, 0.16), 0.12).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)


func _update_visuals() -> void:
	sprite.flip_h = velocity.x < 0.0
	sprite.modulate = Color(1.0, 0.72, 0.72, 1.0) if hit_flash_timer > 0.0 else Color.WHITE
...
func _die() -> void:
	...
	death_tween.tween_property(sprite, "scale", Vector2(0.24, 0.08), 0.2).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN)
