...
static func _normalize_chapter_one_level(level_data: Dictionary) -> void:
	...
	for enemy_variant: Variant in enemies:
		...
		if str(enemy_data.get("type", "")) == "slime":
			enemy_data["type"] = "bat"
			enemy_data["y"] = max(4, int(enemy_data.get("y", 0)) - 4)
		normalized_enemies.append(enemy_data)
	...
	return [
		{
			...
			"triggers": [
				_trigger("c1_l1_intro", 3, 26, 6, 4, "SPACE springt, SHIFT zieht Joey laenger ueber Kanten. Folge dem warmen Licht.", "info", "Kapitel I"),
				_trigger("c1_l1_mid", 35, 24, 8, 5, "Groessere Stufen bedeuten Fallhoehe. Lass Joey abrollen statt blind zu hetzen.", "info"),
		...
		},
		{
			...
			"triggers": [
				_trigger("c1_l4_intro", 3, 26, 10, 4, "Druecke F. Joeys Glow zerschneidet die Dunkelheit und verscheucht Tunnelwuermer.", "info", "Glow"),
		...
		},
		{
			...
			"triggers": [
				_trigger("c1_l6_intro", 4, 44, 8, 4, "Die Hoehle atmet nach oben. Halte Joey in Bewegung und lies jede Stufe frueh.", "info", "Aufstieg"),
				_trigger("c1_l6_top", 69, 12, 8, 5, "Fast geschafft. Wenn du ruhig bleibst, traegt dich die Hoehle selbst.", "reward")
			],
			"worm_count": 0
		},
		{
