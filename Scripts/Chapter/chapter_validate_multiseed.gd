...
const TEST_SEEDS := [101, 237, 404, 777]
...
func _run() -> void:
	...
	for level_index: int in range(total_levels):
		for seed: int in TEST_SEEDS:
			...
			print(
				"MULTI_VALIDATE level=%d seed=%d title=%s rooms=%d branches=%d terrain=%d decor=%d enemies=%d gates=%d invalid_tiles=%d floor=%s path=%s notes=%d status=%s"
				% [level_index + 1, seed, title, room_count, branch_count, terrain_count, decor_count, enemy_count, gate_count, invalid_tile_count, str(touched_floor), str(path_valid), note_count, status]
			)
			if status != "PASS":
