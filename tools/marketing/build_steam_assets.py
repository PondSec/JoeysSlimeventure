...
OUT = ROOT / "marketing/steam/generated"
POSTER_PATH = Path(os.environ["JOEY_STEAM_POSTER_PATH"]) if "JOEY_STEAM_POSTER_PATH" in os.environ else None
STEAM_SET_PATH = ROOT / "marketing/steam/source/steam-set.png"
...
POSTER: Image.Image
STEAM_HERO_PANEL: Image.Image
SCREENSHOT: Image.Image
TORCH_BG: Image.Image
VINE: Image.Image


def load_sources() -> None:
    global POSTER, STEAM_HERO_PANEL, SCREENSHOT, TORCH_BG, VINE
    if POSTER_PATH is None:
        raise SystemExit("Set JOEY_STEAM_POSTER_PATH to the licensed poster source before building Steam art.")
    for source_path in (POSTER_PATH, STEAM_SET_PATH, SCREENSHOT_PATH, TORCH_BG_PATH, VINE_PATH):
        if not source_path.is_file():
            raise SystemExit(f"Missing Steam-art source: {source_path}")
